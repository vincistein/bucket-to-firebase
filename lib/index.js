...
admin.initializeApp();
// functions.storage.bucket("processing-databases").object().onFinalize(
exports.BucketToFirestore = functions.storage.bucket("processing-databases").object().onFinalize(async (object) => {
...
async function setCollectionDatabase(collection, ref) {
    await Promise.all(collection.map((document) => ref.doc().set(document)));
}
